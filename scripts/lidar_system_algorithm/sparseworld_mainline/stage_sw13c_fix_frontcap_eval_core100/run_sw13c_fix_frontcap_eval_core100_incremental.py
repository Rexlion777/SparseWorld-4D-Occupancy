...
REPORTS_DIR = PROJECT_ROOT / "reports/lidar_system_algorithm/sparseworld_mainline/stage_sw13c_fix_frontcap_eval_core100"
LOGS_DIR = PROJECT_ROOT / "logs/lidar_system_algorithm/sparseworld_mainline/stage_sw13c_fix_frontcap_eval_core100"
SCRIPTS_DIR = PROJECT_ROOT / "scripts/lidar_system_algorithm/sparseworld_mainline/stage_sw13c_fix_frontcap_eval_core100"
ARTIFACTS_DIR = PROJECT_ROOT / "artifacts/lidar_system_algorithm/sparseworld_mainline/stage_sw13c_fix_frontcap_eval_core100"
FIGURES_DIR = PROJECT_ROOT / "projects/lidar_system_algorithm/figures/sparseworld_mainline/stage_sw13c_fix_frontcap_eval_core100"
TESTS_DIR = PROJECT_ROOT / "tests/sparseworld_mainline/stage_sw13c_fix_frontcap_eval_core100"
GPU_DUMPS_DIR = ARTIFACTS_DIR / "gpu_phase_dumps"

FRONTCAP50_REPORTS = PROJECT_ROOT / "reports/lidar_system_algorithm/sparseworld_mainline/stage_sw13c_fix_frontcap_eval_core50"
FRONTCAP50_ARTIFACTS = PROJECT_ROOT / "artifacts/lidar_system_algorithm/sparseworld_mainline/stage_sw13c_fix_frontcap_eval_core50"
...
DEFAULT_INHERIT_END = 49
DEFAULT_EVAL_START = 50
DEFAULT_EVAL_END = 99
...
scale50 = load_module("sw13c_fix_frontcap_eval100_scale50", SCALE50_SCRIPT)
frontcap50 = load_module("sw13c_fix_frontcap_eval100_frontcap50", FRONTCAP50_SCRIPT)
sw13c_fix = load_module("sw13c_fix_frontcap_eval100_base", SW13C_FIX_SCRIPT)
sw7 = sw13c_fix.sw7
sw81 = sw13c_fix.sw81
...
def inherited_selected_eval50() -> dict[str, Any]:
    return read_json(FRONTCAP50_REPORTS / "sw13c_fix_frontcap_eval_core50_decision.json")
...
def inherited_selected_rows() -> list[dict[str, str]]:
    rows = read_csv(FRONTCAP50_REPORTS / "sw13c_fix_frontcap_metrics.csv")
    selected = inherited_selected_eval50()
    labels = {
        "A1_fixed": selected["selected_A1_frontcap_result"]["front_cap_variant"],
        "A10_fixed_main": selected["selected_A10_R4_frontcap_result"]["front_cap_variant"],
        "A10_fixed_secondary": selected["selected_A10_R8_frontcap_result"]["front_cap_variant"],
        "C4_fixed": selected["selected_C4_result"]["front_cap_variant"],
    }
...
def write_tests(
    ...
    tests = {
        ...
        "test_protocol_fingerprint.py": """import json\nfrom pathlib import Path\ndef test_protocol_fingerprint():\n    obj=json.loads(Path('/home/rexlion/ComputerVision/cv_lidar_transition/reports/lidar_system_algorithm/sparseworld_mainline/stage_sw13c_fix_frontcap_eval_core100/sw13c_fix_frontcap_eval_core100_protocol_fingerprint.json').read_text())\n    assert obj['protocol_fingerprint_match'] is True\n""",
        ...
        "test_no_rerun_existing_samples.py": f"""import csv\nfrom pathlib import Path\nrows=list(csv.DictReader(Path('/home/rexlion/ComputerVision/cv_lidar_transition/reports/lidar_system_algorithm/sparseworld_mainline/stage_sw13c_fix_frontcap_eval_core100/shard_{new_shard_id}/sw13c_fix_frontcap_replay_manifest_{new_shard_id}.csv').open()))\ndef test_no_rerun_existing_samples():\n    assert rows\n    assert all(int(r['sample_index']) >= {new_label.split('..')[0]} for r in rows)\n    assert all(int(r['sample_index']) <= {new_label.split('..')[1]} for r in rows)\n""",
...
def main() -> None:
    ...
    eval50_decision = inherited_selected_eval50()
    eval50_manifest = read_json(FRONTCAP50_REPORTS / "sw13c_fix_frontcap_frozen_candidate_manifest.json")
    eval50_selection = read_json(FRONTCAP50_REPORTS / "sw13c_fix_frontcap_candidate_selection.json")
    protocol = protocol_fingerprint_payload()
    protocol_match = (
        eval50_manifest["frozen_candidates"] is True
        and eval50_manifest["no_global_reselection"] is True
        and eval50_manifest["no_global_retuning"] is True
        and eval50_manifest["no_training"] is True
        and eval50_selection["selection_uses_gt"] is False
        and eval50_selection["frontcap_selection_only"] is True
        and eval50_selection["selected_A1_frontcap"]["front_cap_variant"] == protocol["A1_frozen"]["front_cap_variant"]
        and eval50_selection["selected_A10_R4_frontcap"]["front_cap_variant"] == protocol["A10_R4_frozen"]["front_cap_variant"]
        and eval50_selection["selected_A10_R8_frontcap"]["front_cap_variant"] == protocol["A10_R8_frozen"]["front_cap_variant"]
        and eval50_selection["selected_C4"]["front_cap_variant"] == protocol["C4_frozen"]["front_cap_variant"]
    )
    write_json(REPORTS_DIR / "sw13c_fix_frontcap_eval_core100_protocol_fingerprint.json", protocol | {"protocol_fingerprint_match": protocol_match})
    write_md(
        REPORTS_DIR / "sw13c_fix_frontcap_eval_core100_protocol.md",
        "\n".join(
            [
                "# eval_core100 FrontCap protocol",
                "",
                f"- eval_core100 is formed by inheriting eval_core50 samples {args.inherit_start}..{args.inherit_end} and incrementally evaluating samples {args.eval_start}..{args.eval_end}.",
    ...
    if not protocol_match:
        ...
        write_json(REPORTS_DIR / "sw13c_fix_frontcap_eval_core100_decision.json", decision)
        write_md(REPORTS_DIR / "sw13c_fix_frontcap_eval_core100_decision.md", "decision: Y6_PROTOCOL_MISMATCH\n")
    ...
    shard_plan = {
        "existing_shard": {
            ...
            "source": "stage_sw13c_fix_frontcap_eval_core50",
    ...
    write_json(REPORTS_DIR / "sw13c_fix_frontcap_eval_core100_shard_plan.json", shard_plan)
    write_md(
        REPORTS_DIR / "sw13c_fix_frontcap_eval_core100_shard_plan.md",
        "\n".join(
            [
                "# eval_core100 incremental shard plan",
                "",
                f"- inherited shard_{inherited_shard_id} from stage_sw13c_fix_frontcap_eval_core50.",
    ...
    write_json(REPORTS_DIR / "sw13c_fix_frontcap_eval_core100_merge_manifest.json", merge_manifest)

    merged_aggregate = sw13c_fix.aggregate_rows(
        merged_rows,
        ["candidate_name", "perturbation_id", "base_repair_variant", "front_cap_variant", "horizon_s"],
    )
    write_csv(REPORTS_DIR / "sw13c_fix_frontcap_eval_core100_merged_metrics.csv", merged_rows)
    write_csv(REPORTS_DIR / "sw13c_fix_frontcap_eval_core100_aggregate_metrics.csv", merged_aggregate)
    write_csv(REPORTS_DIR / "sw13c_fix_frontcap_eval_core100_metric_summary.csv", merged_aggregate)

    inherited_consistency_raw = read_csv(FRONTCAP50_REPORTS / "sw13c_fix_frontcap_sample_consistency.csv")
    ...
    inherited_consistency_selected = [row for row in inherited_consistency_raw if row["front_cap_variant"] == selected_variant_map[row["candidate_name"]]]
    ...
    merged_consistency_rows = [
        *[
            {**row, "partition": "inherited_eval50"}
        ...
        *[
            {**row, "partition": "merged_eval100"}
            for row in merged_consistency_core
        ],
    ]
    write_csv(REPORTS_DIR / "sw13c_fix_frontcap_eval_core100_sample_consistency.csv", merged_consistency_rows)
    write_md(REPORTS_DIR / "sw13c_fix_frontcap_eval_core100_sample_consistency_summary.md", json.dumps(merged_consistency_rows, indent=2))
    ...
    write_csv(REPORTS_DIR / "sw13c_fix_frontcap_eval_core100_density_audit.csv", density_audit_rows)
    write_md(REPORTS_DIR / "sw13c_fix_frontcap_eval_core100_density_audit.md", json.dumps(density_audit_rows, indent=2))
    ...
    for candidate_name in ["A10_fixed_main", "A10_fixed_secondary"]:
        ...
        cons = [row for row in merged_consistency_rows if row["candidate_name"] == candidate_name and row["partition"] == "merged_eval100"][0]
    ...
    write_csv(REPORTS_DIR / "sw13c_fix_frontcap_eval_core100_A10_R4_R8_comparison.csv", comparison_rows)
    write_md(REPORTS_DIR / "sw13c_fix_frontcap_eval_core100_A10_R4_R8_comparison.md", "R8 frozen secondary candidate remains slightly more stable under eval_core100. No eval_core100 search was performed.\n")
    ...
    cons_a1 = next(row for row in merged_consistency_rows if row["candidate_name"] == "A1_fixed" and row["partition"] == "merged_eval100")
    cons_r4 = next(row for row in merged_consistency_rows if row["candidate_name"] == "A10_fixed_main" and row["partition"] == "merged_eval100")
    cons_r8 = next(row for row in merged_consistency_rows if row["candidate_name"] == "A10_fixed_secondary" and row["partition"] == "merged_eval100")
    cons_c4 = next(row for row in merged_consistency_rows if row["candidate_name"] == "C4_fixed" and row["partition"] == "merged_eval100")
    ...
    inherited_summary = {
        "decision": eval50_decision["decision_type"],
        "selected_A1_frontcap": eval50_decision["selected_A1_frontcap_result"]["front_cap_variant"],
        "selected_A10_R4_frontcap": eval50_decision["selected_A10_R4_frontcap_result"]["front_cap_variant"],
        "selected_A10_R8_frontcap": eval50_decision["selected_A10_R8_frontcap_result"]["front_cap_variant"],
    }
    new_shard_summary = {
    ...
    decision = {
        ...
        "sample_coverage": sample_coverage,
        "protocol_fingerprint_match": True,
        "inherited_eval50_summary": inherited_summary,
        f"new_shard_{shard_id}_summary": new_shard_summary,
        "merged_eval100_summary": merged_summary,
        "selected_A1_eval100_result": selected_a1,
        "selected_A10_R4_eval100_result": selected_a10_r4,
        "selected_A10_R8_eval100_result": selected_a10_r8,
        "selected_C4_eval100_result": selected_c4,
    ...
    write_json(REPORTS_DIR / "sw13c_fix_frontcap_eval_core100_decision.json", decision)
    write_md(REPORTS_DIR / "sw13c_fix_frontcap_eval_core100_decision.md", f"decision: {decision_type}\n")

    save_bar_compare(
        FIGURES_DIR / "sw13c_fix_frontcap_eval_core100_density_bar.png",
        f"eval_core100 incremental shard {new_label} frozen FrontCap density audit",
        ["A1", "A10 R4", "A10 R8", "C4"],
        [float(eval50_decision["selected_A1_frontcap_result"]["front_local_density_proxy_after"]), float(eval50_decision["selected_A10_R4_frontcap_result"]["front_local_density_proxy_after"]), float(eval50_decision["selected_A10_R8_frontcap_result"]["front_local_density_proxy_after"]), float(eval50_decision["selected_C4_result"]["front_local_density_proxy_after"])],
    ...
    save_bar_compare(
        FIGURES_DIR / "sw13c_fix_frontcap_eval_core100_recovery_density_tradeoff.png",
        f"eval_core100 incremental shard {new_label} frozen FrontCap recovery-density tradeoff",
        ["A1", "A10 R4", "A10 R8"],
        [abs(float(eval50_decision["selected_A1_frontcap_result"]["front_sector_false_free_rate_delta_vs_native"])), abs(float(eval50_decision["selected_A10_R4_frontcap_result"]["front_sector_false_free_rate_delta_vs_native"])), abs(float(eval50_decision["selected_A10_R8_frontcap_result"]["front_sector_false_free_rate_delta_vs_native"]))],
    ...
    save_line(
        FIGURES_DIR / "sw13c_fix_frontcap_eval_core100_sample_consistency.png",
        f"eval_core100 incremental shard {new_label} frozen FrontCap sample consistency",
        ["A1", "A10 R4", "A10 R8", "C4"],
        [cons_a1["joint_success_rate"], cons_r4["joint_success_rate"], cons_r8["joint_success_rate"], cons_c4["joint_success_rate"]],
    )
    save_bar_compare(
        FIGURES_DIR / "sw13c_fix_frontcap_eval_core100_A10_R4_R8_comparison.png",
        f"eval_core100 incremental shard {new_label} frozen FrontCap A10 R4 vs R8",
    ...
    save_bar_compare(
        FIGURES_DIR / "sw13c_fix_frontcap_eval_core100_shard_comparison.png",
        f"eval_core100 incremental shard {new_label} frozen FrontCap shard comparison",
        ["A1", "A10 R4", "A10 R8", "C4"],
    ...
    if "A1_fixed" in visual_cases:
        save_bev_panel(
            FIGURES_DIR / f"sw13c_fix_frontcap_eval_core100_bev_A1_sample{new_visual_sample}.png",
            ...
            f"eval_core100 incremental shard {new_label} frozen candidate frozen FrontCap no GT front cap no training subset diagnostic",
        )
    if "A10_fixed_main" in visual_cases:
        save_bev_panel(
            FIGURES_DIR / f"sw13c_fix_frontcap_eval_core100_bev_A10_sample{new_visual_sample}.png",
    ...
    report_md = "\n".join(
        [
            "# Stage SW-13C-Fix-FrontCap eval_core100",
            "",
            "1. Executive summary",
            f"- decision: {decision_type}",
            "",
            "2. Incremental protocol",
            f"- eval_core100 is formed by inheriting eval_core50 samples {inherited_label} and incrementally evaluating samples {new_label}.",
            "",
            "3. Protocol fingerprint verification",
            f"- protocol fingerprint matches = {protocol_match}",
            "",
            "4. Inherited eval_core50 summary",
            f"- inherited decision: {eval50_decision['decision_type']}",
            "",
            f"5. New shard {new_label} summary",
            f"- sample count: {len(eval_ids)}",
            "",
            "6. Merged eval_core100 result",
            f"- sample coverage: {sample_coverage['total_unique_samples']}",
            "",
            "7. A1 eval_core100 result",
            f"- front cap: {protocol_by_name['A1_fixed'].front_cap_variant}",
            "",
            "8. A10 R4 eval_core100 result",
            f"- front cap: {protocol_by_name['A10_fixed_main'].front_cap_variant}",
            "",
            "9. A10 R8 eval_core100 result",
            ...
            f"- inherited eval50, new shard {new_label}, and merged eval100 are reported separately.",
            "",
            "13. A10 R4 vs R8 comparison",
            "- R8 frozen secondary candidate remains slightly more stable under eval_core100.",
            "",
            "14. Decision Y1-Y6",
            f"- {decision_type}",
            "",
            "15. Safe claims",
            (
                "- Frozen no-GT FrontCap candidates scale from eval_core50 to eval_core100 with strong front-sector recovery and controlled front-local density risk."
                if decision_type == "Y1_FRONTCAP_EVAL100_STRONG"
                else "- Frozen no-GT FrontCap candidates maintain medium positive direction on eval_core100; results support diagnostic resume claims but not official benchmark claims."
    ...
    write_md(REPORTS_DIR / "stage_sw13c_fix_frontcap_eval_core100_report.md", report_md)
    write_json(REPORTS_DIR / "stage_sw13c_fix_frontcap_eval_core100_report.json", {"decision": decision_type, "sample_coverage": sample_coverage})
